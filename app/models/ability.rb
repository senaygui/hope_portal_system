class Ability
  ...
  def initialize(user)
    ...
    when 'instructor'
      ...
      can :read, Notice
      can :read, StudentGrade, course_id: Course.instructor_courses(user.id).pluck(:id)
      can :update, StudentGrade, course_id: Course.instructor_courses(user.id).pluck(:id)
